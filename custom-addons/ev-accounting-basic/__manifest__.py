{
    "name": "EV Accounting Basic",
    "version": "18.0.1.0.0",
    "category": "Accounting",
    "summary": "EV Accounting Basic",
    "description": """
        This module provides basic features for accounting in Odoo.
    """,
    "author": "Experts Vision Company",
    "website": "https://expertsvision.org",
    "depends": ["base", "account", "sale_management", "stock", "purchase", "stock"],
    "data": [],
    "demo": [],
    "application": True,
    "license": "AGPL-3",
    "images": ["static/description/icon.jpeg"],
    "support": "support@expertsvision.org",
    # 'installable': True,
    # 'auto_install': False,
    # 'currency': 'EUR',
    # 'maintainer': 'Maintainer Name',
    # 'external_dependencies': {
    #     'python': [],  # Python libraries required
    #     'bin': [],  # External binaries required
    # },
    # 'pre_init_hook': 'pre_init_hook_method',  # Hook to execute before module installation
    # 'post_init_hook': 'post_init_hook_method',  # Hook to execute after module installation
    # 'post_load_hook': 'post_load_hook_method',  # Hook to execute after loading the module
}
